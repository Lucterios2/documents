...
class DocumentsMigrate(MigrateAbstract):
    ...
    def _folders(self):
        ...
        for folderid, folder_name, folder_description, folder_parent in cur.fetchall():
            ...
            self.folder_list[folderid] = folder_mdl.objects.create(name=folder_name, description=folder_description)
            if folder_parent is not None:
                self.folder_list[folderid].parent = self.folder_list[folder_parent]
            ...
            self.folder_list[folderid].modifier = group_mdl.objects.filter(id__in=modif_group)
            self.folder_list[folderid].viewer = group_mdl.objects.filter(id__in=visu_group)
    ...
    def _docs(self):
        ...
        for docid, doc_name, doc_description, doc_folder, doc_modifier, doc_creator, doc_datemod, doc_datecreat in cur.fetchall():
            ...
            self.doc_list[docid] = doc_mdl.objects.create(name=doc_name, description=doc_description, date_modification=doc_datemod, date_creation=doc_datecreat)
            ...
            new_filename = get_user_path("documents", "document_%s" % six.text_type(self.doc_list[docid].id))
            old_filename = join(self.old_db.tmp_path, "usr", "org_lucterios_documents", "document%d" % docid)
